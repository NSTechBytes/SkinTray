...
namespace SkinTray
{
    public class Measure
    {
        ...
        public void Reload(API api, ref double maxValue)
        {
            ...
            // Attempt to load the icon file with error handling
            try
            {
                ...
                {
                    // Log error using the static API.Log method with a log level argument
                    API.Log(1, "Icon path is empty. Please provide a valid icon path in the Rainmeter config file.");
                }
            }
            catch (Exception ex)
            {
                // Log the error using the static API.Log method with an appropriate log level
                API.Log(1, $"Error loading icon from path '{_iconPath}': {ex.Message}");
                // Optionally, set a default icon here if available.
            }
